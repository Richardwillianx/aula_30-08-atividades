...
class BombaCombustivel {
  ...
  abastecerPorValor(valor: number) {
    ...
    if (this.quantidadeCombustivel >= litrosCombustivel) {
      ...
      console.log(
        `com R$${valor} você abasteceu ${litrosCombustivel}L e restam ${this.quantidadeCombustivel}L na bomba`
      );
      return;
    }
  ...
  abastecerPorLitro(litros: number) {
    if (this.quantidadeCombustivel >= litros) {
      ...
      console.log(
        `Você abasteceu ${litros} e deverá pagarR$${valorFinal} e restam ${this.quantidadeCombustivel}L na bomba`
      );
      return;
    }
  ...
  alterarCombustivel(combustível: string) {
    this.tipoCombustivel = combustível;
  }

  alterarQuantidadeCombustivel(quantidadeTotal: number) {
